...
- **Fixed** `vp run --cache` now supports running without a task specifier and opens the interactive task selector, matching bare `vp run` behavior ([#???](https://github.com/voidzero-dev/vite-task/pull/???))
